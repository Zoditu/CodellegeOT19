...
function Search( criteria, initialID )
{
    SearchCriteria.loading = true;
    GET
    (
        host,
        music_songs,
        { initialid: initialID, max: MAX_QUERY },
        { search: criteria },
        function( result, error, request ) 
        { 
            //Estoy cargando datos
            ...
            else if( !error )
            {
                ...
                $.each( songs.data.data, ( i, song ) =>
                {
                    SearchCriteria.list.songs.push( song );
                });

                if( songs.data.totalResults === 0 )
                {
                    BackgroundControl.visible = false;
                    SearchCriteria.loading = false;
                }
                else
                    Search( criteria, songs.data.lastIndex + 1 );
            }
            else
            {
                SearchCriteria.loading = false;
